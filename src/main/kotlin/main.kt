...
fun main(args: Array<String>) {
    ...
    val ALGORITHMS = listOf<IAlgorithm>(BlindRandomSearch(), LocalSearch(), SimulatedAnnealing(), SimulatedAnnealingBetter())
    val INSTANCES = listOf<Instance>(CALL_007_VEHICLE_03, CALL_018_VEHICLE_05, CALL_035_VEHICLE_07, CALL_080_VEHICLE_20, CALL_130_VEHICLE_40)
...
fun runAlgo(instance: Instance, search: IAlgorithm):Result {
    ...
    val impro = (costFunction(initSol.instance, initSol.arr).toDouble()-bestSol.cost)/costFunction(initSol.instance, initSol.arr)
    val averageRuntime = (sumtime/10)/1000
    return Result(name, search.name, (averageCost/10).toLong(),bestSol.cost, String.format("%.3f", impro*100).toDouble(),"%.4f".format(averageRuntime), bestSol.instance, bestSol.solution)
}
